...
static const float CbCr_mult = 256.0 / 255.0;
static const float CbCr_sub = 128.501895 / 255.0;

[numthreads(1, 1, 1)]
void rgb_main(uint2 dispatchThreadId : SV_DispatchThreadID)
{
    output[dispatchThreadId] = input[uint2(PixelOffset.x + dispatchThreadId.x, PixelOffset.y - dispatchThreadId.y)];
}

[numthreads(1, 1, 1)]
void yacbcr_main(uint2 dispatchThreadId : SV_DispatchThreadID)
{
    float4 YACbCr;
